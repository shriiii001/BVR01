...
def runPPO(args):
    ...
    for i_episode in range(1, args['Eps']+1):
        ...
        # tb_obs0 = None          =====cmt on 19jan 22:00
        tb_obs0 = {}                            #----==== add on 19jan 22:00

        for obs in tb_obs:                       #----==== add on 19jan 22:00
            for key, val in obs.items():          #----==== add on 19jan 22:00
                if key not in tb_obs0:               #----==== add on 19jan 22:00
                    tb_obs0[key] = 0                    #----==== add on 19jan 22:00
                tb_obs0[key] += val                        #----==== add on 19jan 22:00s
        ...
        # Behaviour-level metrics (averaged across workers)
        if tb_obs0 is not None:
            if 'Blue_alive' in tb_obs0:
                writer.add_scalar("metrics/blue_survival", tb_obs0['Blue_alive'], i_episode)
            if 'Red_alive' in tb_obs0:
                writer.add_scalar("metrics/red_survival", tb_obs0['Red_alive'], i_episode)
            if 'maxTime' in tb_obs0:
                writer.add_scalar("metrics/max_time_reached", tb_obs0['maxTime'], i_episode)
...
def train(args):
    ...
    ppo.policy_old.eval()
    running_reward = 0.0
    # episode_return = 0.0              #------------------==== add on 19jan 21:31
    ...
    for i_episode in range(1, args[0]['eps']+1):
        ...
        while not done:
            ...
            episode_return += reward                   #------------------==== add on 19jan 21:31           

        running_reward += episode_return                #------------------==== add on 19jan 21:31
